...
const CourseBatchNavbar = () => {
  ...
  function FetchPending() {
    fetch("https://xapi.learn2read.co/api/tutors/get_tutor_courses?IsTutorAccepted=2&Status=1"
      , {
        headers: {
      ...
      .then((data) => {
        ...
        setBatchRequests(data.TutorCourses);
      })
      .catch((error) => {
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th style={{ border: '1px solid #ddd', backgroundColor: '#1E87DB', padding: '8px', color: 'white', fontWeight: '400', textAlign: 'center' }}>CATEGORY</th>
                  <th style={{ border: '1px solid #ddd', backgroundColor: '#1E87DB', padding: '8px', color: 'white', fontWeight: '400', textAlign: 'center' }}>AGE GROUP</th>
                  <th style={{ border: '1px solid #ddd', backgroundColor: '#1E87DB', padding: '8px', color: 'white', fontWeight: '400', textAlign: 'center' }}>LANGUAGE</th>
                  <th style={{ border: '1px solid #ddd', backgroundColor: '#1E87DB', padding: '8px', color: 'white', fontWeight: '400', textAlign: 'center' }}>CLASS PER WEEK</th>
              ...
              <tbody>
                {
                  batchRequests.map((batch, index) => (
                    <tr key={index}>
                      ...
                      <td>{batch.FromAge}-{batch.ToAge}</td>
                      <td>{batch.Language}</td>
                      <td>{batch.ClassPerWeek}</td>
                      <td><button className='lesson-plan-btn' onClick={() => navigate(`/lessonplan/${batch.CourseID}`)}>Lesson Plan</button></td>
                    </tr>
                  ))
                }
              </tbody>
...
export default CourseBatchNavbar;
